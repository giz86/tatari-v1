...
if($user_info[0]->user_role_id==1){
	...
	if(!isset($month_year)){
		...
		$month = date('m', $date);
		$year = date('Y', $date);
		$tat_employees = $this->Attendance_model->get_tat_employees();
	} else {
		...
		if($this->input->post('employee_id')==0){
			...
			$tat_employees = $this->Tat_model->read_user_info($this->input->post('employee_id'));
		}
	}
	...
	$tat_employees = $this->Tat_model->read_user_info($session['user_id']);
}
...
<div class="box mb-4 <?php echo $get_animate;?>">
  <div class="box-body">
    <div class="row">
      <div class="col-md-12">
        <?php $attributes = array('name' => 'tat-form', 'id' => 'tat-form', 'autocomplete' => 'off');?>
        <?php $hidden = array('_user' => $session['user_id']);?>
        <?php echo form_open('admin/attendance/', $attributes, $hidden);?>
        <div class="row">
          <div class="col-md-2">
            <div class="form-group">
              <label for="first_name"><?php echo $this->lang->line('tat_e_details_date');?></label>
          ...
          <div class="col-md-3">
            <div class="form-group">
              ...
              <select class="form-control" name="company_id" id="aj_company" data-plugin="select_tat" data-placeholder="<?php echo $this->lang->line('left_company');?>" required>
                <option value=""></option>
          ...
          <div class="col-md-3">
            <div class="form-group" id="employee_ajax">
              ...
              <select name="employee_id" id="employee_id" class="form-control employee-data" data-plugin="select_tat" data-placeholder="<?php echo $this->lang->line('tat_choose_an_employee');?>" required>
          ...
          <div class="col-md-3">
            <div class="form-group">
              ...
              <br />
              <?php echo form_button(array('name' => 'tatari_form', 'type' => 'submit', 'class' => 'btn btn-primary', 'content' => '<i class="fa fa fa-check-square-o"></i> '.$this->lang->line('tat_get'))); ?> </div>
          </div>
        </div>
...
<div class="box <?php echo $get_animate;?>">
  <div class="box-header with-border">
    <h3 class="box-title"> <?php echo $this->lang->line('tat_employees_monthly_timesheet');?></h3>
    <h5>For the month of
      <?php if(isset($month_year)): echo date('F Y', strtotime($month_year)); else: echo date('F Y'); endif;?>
    </h5>
    <div class="box-tools pull-right"> A: Absent, P: Present, L: Leave</div>
  </div>
  <div class="box-body">
      ...
      <table class="datatables-demo table table-striped table-bordered" id="tat_table">
        <thead>
          <tr>
            ...
            <?php endfor; ?>
            <th width="100px"><?php echo $this->lang->line('tat_timesheet_workdays');?></th>
          </tr>
        </thead>
        <tbody>

          <?php $j=0;foreach($tat_employees as $r):?>
          <?php
		  	// Name
			...
			?>
          <?php $employee_name = $full_name.'<br><small class="text-muted"><i>'.$department_designation.'<i></i></i></small><br><small class="text-muted"><i>'.$this->lang->line('tat_employees_id').': '.$r->employee_id.'<i></i></i></small>';?>
          <tr>
			...
			// get holiday
			// $h_date_chck = $this->Attendance_model->holiday_date_check($attendance_date);
			// $holiday_arr = array();
			// if($h_date_chck->num_rows() == 1){
			// 		// $h_date = $this->Attendance_model->holiday_date($attendance_date);
