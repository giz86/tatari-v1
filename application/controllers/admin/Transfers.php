...
class Transfers extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('tat_transfers').' | '.$this->Tat_model->site_title();
		...
		$data['breadcrumbs'] = $this->lang->line('tat_transfers');
    ...
    public function transfer_list()
     {
        ...
        foreach($transfer->result() as $r) {
			...
			if(in_array('211',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-transfer_id="'. $r->transfer_id . '"><span class="fa fa-pencil"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('212',$role_resources_ids)) { //delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->transfer_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			if(in_array('233',$role_resources_ids)) { //view
				$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-transfer_id="'. $r->transfer_id . '"><span class="fa fa-eye"></span></button></span>';
			} else {
				$view = '';
			}
		$combhr = $edit.$view.$delete;
		$iemployee_name = $employee_name.'<br><small class="text-muted"><i>'.$this->lang->line('tat_transfer_to_department').': '.$department_name.'<i></i></i></small><br><small class="text-muted"><i>'.$this->lang->line('tat_transfer_to_location').': '.$location_name.'<i></i></i></small>';
	...
	public function add_transfer() {
		...
		if($this->input->post('company_id')==='') {
			...
			$Return['error'] = $this->lang->line('tat_transfers_error_date');
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_transfers_success_updated');
		} else {
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('tat_transfers_success_deleted');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
