...
class Termination extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('left_terminations').' | '.$this->Tat_model->site_title();
    ...
    public function termination_list()
     {
		...
		// get user > warning to
		$euser = $this->Tat_model->read_user_info($r->employee_id);
		// user full name
		if(!is_null($euser)){
		...
		$termination_date = $this->Tat_model->set_date_format($r->termination_date);
		...
		if(in_array('229',$role_resources_ids)) { //edit
			$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-termination_id="'. $r->termination_id . '"><span class="fa fa-pencil"></span></button></span>';
		} else {
			$edit = '';
		}
		if(in_array('230',$role_resources_ids)) { // delete
			$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->termination_id . '"><span class="fa fa-trash"></span></button></span>';
		} else {
			$delete = '';
		}
		if(in_array('239',$role_resources_ids)) { //view
			$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-termination_id="'. $r->termination_id . '"><span class="fa fa-eye"></span></button></span>';
		} else {
			$view = '';
		}
	...
	public function add_termination() {
		...
		if($this->input->post('company_id')==='') {
			...
			$Return['error'] = $this->lang->line('tat_error_resignation_notice_date');
			 ...
			 $Return['error'] = $this->lang->line('tat_error_termination_date');
		} else if($nt_date > $tt_date) {
        	$Return['error'] = $this->lang->line('tat_error_termination_notice_date_less');
		} else if($this->input->post('type')==='') {
			 $Return['error'] = $this->lang->line('tat_error_termination_type');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_termination_added');
		} else {
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_termination_updated');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('tat_success_termination_deleted');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
