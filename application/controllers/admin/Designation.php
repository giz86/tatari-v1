<?php
// Designation Controller - interact with model and view regarding designation
...
class Designation extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('tat_designations').' | '.$this->Tat_model->site_title();
		$data['all_departments'] = $this->Department_model->all_departments();
		$data['get_all_companies'] = $this->Tat_model->get_companies();
		//$data['all_designations'] = $this->Designation_model->all_designations();
		$data['breadcrumbs'] = $this->lang->line('tat_designations');
          ...
          foreach($designation->result() as $r) {
			...
			if($system[0]->is_active_sub_departments=='yes'){
				...
				if(!is_null($subdepartment)){
					...
					$subdep_name = '<br><small class="text-muted"><i>'.$this->lang->line('tat_hr_sub_department').': '.$subdep_name.'<i></i></i></small>';
				} else {
					$subdep_name = '<br><small class="text-muted"><i>'.$this->lang->line('tat_hr_sub_department').': --<i></i></i></small>';	
				}
			} else {
				$subdep_name ='';
			}
			...
			if(in_array('244',$role_resources_ids)) { //Update
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target="#edit-modal-data"  data-designation_id="'. $r->designation_id . '"><span class="fa fa-pencil"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('245',$role_resources_ids)) { //Delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->designation_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			
			$combhr = $edit.$delete;
			$idesignation_name = $r->designation_name.'<br><small class="text-muted"><i>Department: '.$department_name.'<i></i></i></small>'.$subdep_name.'';
	...
	public function add_designation() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_add_designation');
		} else {
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_update_designation');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if($this->input->post('is_ajax')==2) {
			...
			if(isset($id)) {
				$Return['result'] = $this->lang->line('tat_success_delete_designation');
			} else {
				$Return['error'] = $this->lang->line('tat_error_msg');
			}
			$this->output($Return);
