<?php
/**
 *  Invoices Controller : Billing and Invoice Management for Tatari System - Finance Module
 * 
 */
...
class Invoices extends MY_Controller
{
	...
	public function index() {
		...
		$data['title'] = $this->lang->line('tat_invoices_title').' | '.$this->Tat_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('tat_invoices_title');
		$data['all_taxes'] = $this->Tax_model->get_all_taxes();
		$data['path_url'] = 'tatari_invoices';
	...
	public function create() {
		...
		$data['title'] = $this->lang->line('tat_invoice_create').' | '.$this->Tat_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('tat_invoice_create');
		$data['all_taxes'] = $this->Tax_model->get_all_taxes();
		$data['path_url'] = 'create_tatari_invoice';
		$role_resources_ids = $this->Tat_model->user_role_resource();
	...
	public function taxes()
     {
		$data['title'] = $this->lang->line('tat_invoice_tax_types').' | '.$this->Tat_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('tat_invoice_tax_types');
	...
	public function payments_history() {
		...
		$data['title'] = $this->lang->line('tat_acc_invoice_payments').' | '.$this->Tat_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('tat_acc_invoice_payments');
		$data['path_url'] = 'tat_invoice_payment';
		$role_resources_ids = $this->Tat_model->user_role_resource();
		if(in_array('121',$role_resources_ids)) {
			if(!empty($session)){ 
	...
	public function invoice_payment_list()
     {
		...
		$balance2 = 0;
          foreach($transaction->result() as $r) {
			...
			$issued_to = $invoice_info[0]->issued_to;
			if($issued_to==""){
			   $issued_to='Undisclosed N/A';
			 }
	 ...
	 public function edit() {
		...
		$company = $this->Tat_model->read_company_setting_info(1);

		$ccountry = $this->Tat_model->read_country_info($company[0]->country);
		$data = array(
			'title' => $this->lang->line('tat_title_edit_invoice').' '.$invoice_info[0]->invoice_id,
			'breadcrumbs' => $this->lang->line('tat_title_edit_invoice'),
			'path_url' => 'create_tatari_invoice',
	...
	public function view() {
		...
		$data = array(
			'title' => $this->lang->line('tat_view_invoice').' ' .$invoice_info[0]->invoice_id,
			'breadcrumbs' => $this->lang->line('tat_view_invoice'),
			'path_url' => 'create_tatari_invoice',
			'invoice_id' => $invoice_info[0]->invoice_id,
			...
			'name' => $invoice_info[0]->issued_to,
			'client_company_name' => $invoice_info[0]->issued_to,
	...
	public function update_invoice_status() {
		...
		if($this->input->post('status') == 1){
			...
			'payer_payee_id' => $invoice[0]->issued_to,
			'payment_method_id' => 3,
			'description' => 'Invoice Payments',
			'reference' => 'Invoice Payments',
			'invoice_id' => $id,
			'client_id' => $invoice[0]->issued_to,
			'created_at' => date('Y-m-d H:i:s')
			);
			$this->Finance_model->add_transactions($ivdata);
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('tat_invoice_has_been_paid');
				$this->session->set_flashdata('response',$this->lang->line('tat_invoice_has_been_paid'));
			} else {
				$Return['error'] = $this->lang->line('tat_error_msg');
			}
		} else {
			$Return['result'] = $this->lang->line('tat_invoice_still_un_paid');
			$this->session->set_flashdata('response',$this->lang->line('tat_invoice_still_un_paid'));
		}
	...
	public function invoices_list()
     {
		...
		$data = array();
          ...
          foreach($client->result() as $r) {
			 ...
			 $grand_total = $this->Tat_model->currency_sign($r->grand_total);
			 ...
			 if($issued_to==""){
			  ...
			  $invoice_date = '<i class="far fa-calendar-alt position-left"></i> '.$this->Tat_model->set_date_format($r->invoice_date);
			  $invoice_due_date = '<i class="far fa-calendar-alt position-left"></i> '.$this->Tat_model->set_date_format($r->invoice_due_date);
			  ...
			  if(in_array('328',$role_resources_ids)) { 
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><a href="'.site_url().'admin/invoices/edit/'.$r->invoice_id.'/"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-pencil"></span></button></a></span>';
			} else {
				$edit = '';
			}
			if(in_array('329',$role_resources_ids)) { 
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->invoice_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			if(in_array('330',$role_resources_ids)) {
				$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_view').'"><a href="'.site_url().'admin/invoices/view/'.$r->invoice_id.'/"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light""><span class="fa fa-arrow-circle-right"></span></button></a></span>';
			} else {
				$view = '';
			}
			if(in_array('330',$role_resources_ids)) { 
				$qstatus = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_change_status').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light delete" data-toggle="modal" data-target=".add-modal-data" data-invoice_id="'. $r->invoice_id . '"><span class="fa fa-pencil-square-o"></span></button></span>';
			} else {
				$qstatus = '';
			}
			if($r->status == 0){
				...
				$status = '<span class="label label-success">'.$this->lang->line('tat_payment_paid').'</span>';
				$combhr = $view.$delete;
			} else {
				$status = '<span class="label label-info">'.$this->lang->line('tat_acc_inv_cancelled').'</span>';
				$combhr = $edit.$view.$qstatus.$delete;
			}
	...
	public function create_new_invoice() {
		...
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		 ...
		 if($this->input->post('invoice_number')==='') {
			...
			$Return['error'] = "Unit price field is required.";
		}
	 ...
	 public function taxes_list()
     {
		...
		$data = array();
          ...
          foreach($taxes->result() as $r) {
				...
				if($r->type == 'fixed'): $type = $this->lang->line('tat_title_tax_fixed'); else: $type = $this->lang->line('tat_title_tax_percent'); endif;
