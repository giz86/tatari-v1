<?php
// Department Controller - to interact with input and model regarding departments and sub-departments.
...
class Department extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('tat_departments').' | '.$this->Tat_model->site_title();
		$data['all_locations'] = $this->Tat_model->all_locations();
		$data['all_employees'] = $this->Tat_model->all_employees();
		$data['get_all_companies'] = $this->Tat_model->get_companies();
		$session = $this->session->userdata('username');
		$data['breadcrumbs'] = $this->lang->line('tat_departments');
	 ...
	 public function sub_departments()
     {
		...
		$data['title'] = $this->lang->line('tat_hr_sub_departments').' | '.$this->Tat_model->site_title();
		$session = $this->session->userdata('username');
		$data['breadcrumbs'] = $this->lang->line('tat_hr_sub_departments');
     ...
     public function department_list() {
		...
		$data = array();
          ...
          foreach($department->result() as $r) {
			...
			// get user > head
			$head_user = $this->Tat_model->read_user_info($r->employee_id);
			// user full name
			...
			if(in_array('241',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target="#edit-modal-data"  data-department_id="'. $r->department_id . '"><span class="fa fa-pencil"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('242',$role_resources_ids)) { // delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->department_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
			$ititle = $r->department_name.'<br><small class="text-muted"><i>Head: '.$dep_head.'<i></i></i></small>';
			$combhr = $edit.$delete;
    ...
    public function sub_department_list() {
		...
		$data = array();
          ...
          foreach($department->result() as $r) {
			...
			if(in_array('241',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target="#edit-modal-data"  data-department_id="'. $r->sub_department_id . '"><span class="fa fa-pencil"></span></button></span>';
			} else {
				$edit = '';
			}
			if(in_array('242',$role_resources_ids)) { // delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->sub_department_id . '"><span class="fa fa-trash"></span></button></span>';
			} else {
				$delete = '';
			}
	 ...
	 public function read()
	{
		...
		if(is_numeric($keywords[0])) {
			...
			$data = array(
				...
				'all_locations' => $this->Tat_model->all_locations(),
    ...
    public function add_department() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_add_department');
		} else {
	...
	public function add_sub_department() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_hr_sub_department_added');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
	...
	public function update() {
		...
		if(is_numeric($keywords[0])) {
			...
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('tat_success_update_department');
			} else {
				$Return['error'] = $this->lang->line('tat_error_msg');
			}
			$this->output($Return);
	...
	public function update_sub_record() {
		...
		if(is_numeric($keywords[0])) {
			...
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('tat_hr_sub_department_updated');
			} else {
				$Return['error'] = $this->lang->line('tat_error_msg');
			}
			$this->output($Return);
	...
	public function delete() {
		...
		if($this->input->post('is_ajax')==2) {
			...
			if(is_numeric($keywords[0])) {
				...
				if(isset($id)) {
					$Return['result'] = $this->lang->line('tat_success_delete_department');
				} else {
					$Return['error'] = $this->lang->line('tat_error_msg');
				}
				$this->output($Return);
	...
	public function sub_delete() {
		...
		if($this->input->post('is_ajax')==2) {
			...
			if(is_numeric($keywords[0])) {
				...
				if(isset($id)) {
					$Return['result'] = $this->lang->line('tat_hr_sub_department_deleted');
				} else {
					$Return['error'] = $this->lang->line('tat_error_msg');
				}
				$this->output($Return);
