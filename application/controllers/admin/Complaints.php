...
class Complaints extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('left_complaints').' | '.$this->Tat_model->site_title();
    ...
    public function complaint_list()
     {
        ...
        foreach($complaint->result() as $r) {
				...
				if(in_array('223',$role_resources_ids)) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-complaint_id="'. $r->complaint_id . '"><span class="fa fa-pencil"></span></button></span>';
				} else {
					$edit = '';
				}
				if(in_array('224',$role_resources_ids)) { // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->complaint_id . '">
					<span class="fa fa-trash"></span></button></span>';
				} else {
					$delete = '';
				}
				if(in_array('237',$role_resources_ids)) { //view
					$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-complaint_id="'. $r->complaint_id . '"><span class="fa fa-eye"></span></button></span>';
				} else {
					$view = '';
				}
				// get company
	...
	public function add_complaint() {
		...
		if($this->input->post('company_id')==='') {
			$Return['error'] = $this->lang->line('error_company_field');
       		 ...
       		 $Return['error'] = $this->lang->line('tat_error_complaint_from');
		} else if($this->input->post('title')==='') {
			$Return['error'] = $this->lang->line('tat_error_complaint_title');
		} else if($this->input->post('complaint_date')==='') {
			 $Return['error'] = $this->lang->line('tat_error_complaint_date');
		} else if($this->input->post('complaint_against')==='') {
			 $Return['error'] = $this->lang->line('tat_error_complaint_against');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_complaint_added');
		} else {
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('tat_success_complaint_deleted');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
	...
	public function update() {
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_complaint_updated');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
