...
class Resignation extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('left_resignations').' | '.$this->Tat_model->site_title();
    ...
    public function resignation_list()
     {
		...
		// get notice date
		$notice_date = $this->Tat_model->set_date_format($r->notice_date);
		// get resignation date
		$resignation_date = $this->Tat_model->set_date_format($r->resignation_date);
		// get company
		...
		if(in_array('214',$role_resources_ids)) { //edit
			$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-resignation_id="'. $r->resignation_id . '"><span class="fa fa-pencil"></span></button></span>';
		} else {
			$edit = '';
		}
		if(in_array('215',$role_resources_ids)) { // delete
			$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->resignation_id . '"><span class="fa fa-trash"></span></button></span>';
		} else {
			$delete = '';
		}
		if(in_array('234',$role_resources_ids)) { //view
			$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('tat_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-resignation_id="'. $r->resignation_id . '"><span class="fa fa-eye"></span></button></span>';
		} else {
			$view = '';
		}
		...
		$iemployee_name = $employee_name.'<br><small class="text-muted"><i>'.$this->lang->line('tat_reason').': '.$r->reason.'<i></i></i></small>'.'<br><small class="text-muted"><i>'.$app_status.'<i></i></i></small>';
	...
	public function add_resignation() {
		...
		if($this->input->post('company_id')==='') {
			...
			$Return['error'] = $this->lang->line('tat_error_resignation_notice_date');
		} else if($this->input->post('resignation_date')==='') {
			 $Return['error'] = $this->lang->line('tat_error_resignation_date');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_resignation_added');			
		} else {
	...
	public function update() {
		...
		if($this->input->post('notice_date')==='') {
			$Return['error'] = $this->lang->line('tat_error_resignation_notice_date');
			 ...
			 $Return['error'] = $this->lang->line('tat_error_template_status');
		}
		...
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('tat_success_resignation_updated');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
	...
	public function delete() {
		...
		if(isset($id)) {
			$Return['result'] = $this->lang->line('tat_success_resignation_deleted');
		} else {
			$Return['error'] = $this->lang->line('tat_error_msg');
		}
		$this->output($Return);
